(function($) {
	$.fn.facebook_wall = function(options) {
		...
		options = $.extend({
			...
			locale: 'ar_EG', // your contry code
			avatar_size: 'square', // square | small | normal | large
			message_length: 500, // Any amount you like. Above 0 shortens the message length
						...
						if (this.type === 'link' || this.type === 'photo' || this.type === 'video') {
							...
							output += '<div class="media' + media_class + ' clearfix">';
								if (this.picture !== undefined) {
									output += '<div class="image"><a href="' + this.link + '" target="_blank"><img src="' + (graphURL + this.object_id + '/picture?type=normal') + '" /></a></div>';
								} else if (this.object_id !== undefined) {
									output += '<div class="image"><a href="' + this.link + '" target="_blank"><img src="' + (graphURL + this.object_id + '/picture?type=large') + '" /></a></div>';
								}
